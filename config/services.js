...
//    "NOOLAP select_from_compare ": "select element_code, element_label, year, coalesce(value, -1), um from ( select element, year, value, um from compare where data_source = '{SOURCE}' and fertilizer = '{PRODUCT}' and country = '{COUNTRY}' and n_p = '{KIND}' ) c right join codes_elements on element = element_code ORDER BY element ASC, year ASC",
    //"NOOLAP select_from_compare_ifa": "select element_code, element_label, year, coalesce(value, -1), um from ( select element, year, value, um from country_stat where fertilizer = '{PRODUCT}' and country = '{COUNTRY}' and n_p = '{KIND}' ) c right join codes_elements on element = element_code ORDER BY element ASC, year ASC",

    "select_from_compare": "select element_code, element_label, CASE WHEN year is null then '' else cast ( year as character varying) end as year, CASE WHEN value is null then '' else cast ( value as character varying) end  as value, CASE WHEN um is null then '' else cast ( um as character varying) end  as Unit, '' as Flag from ( select element, year, value, um from compare where data_source = '{SOURCE}' and fertilizer = '{PRODUCT}' and country = '{COUNTRY}' and n_p = '{KIND}' ) c right join codes_elements on element = element_code ORDER BY element ASC, year ASC",
